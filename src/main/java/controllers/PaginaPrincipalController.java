...
public class PaginaPrincipalController {
	...
	int imprimirPeliculas(String type, int totalAnterior) throws IOException {
		...
		if (type.equals("genre")) {
			url = "https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=es-ES&page=1&sort_by=popularity.desc&with_genres=" + userGenre;
		} else {
		...
		if (type.equals("popular")) {
			...
			imprimirLabel("Ahora en cines", (66.0 * 3) + Math.floor(totalAnterior / imagenesPorFila) * saltoY);
		} else if (type.equals("upcoming")) {
			imprimirLabel("Próximos lanzamientos", (66.0 * 2) + Math.floor(totalAnterior / imagenesPorFila) * saltoY);
		} else if (type.equals("genre")) {
			imprimirLabel("Basado en tus gustos", 66.0 + Math.floor(totalAnterior / imagenesPorFila) * saltoY);
		}
